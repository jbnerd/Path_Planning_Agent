...
# Class definitions used for Breadth First Search Algorithm
import copy
...
class Node(object):
	...
	def __init__(self, state, parent, action, path_cost, matrix_size):
		"""
	...
	def children(self, matrix_size):
		...
		if self.state.dirt_list is not None and self.state.agent_coordinate in self.state.dirt_list:
			...
			new_action = "suck"
			new_node = Node(new_state, self, new_action, new_path_cost, matrix_size)
			...
			for action in actions:
				if action == "up" and self.state.agent_coordinate[0] != 0:
					...
					child_nodes.append(Node(State([self.state.agent_coordinate[0] - 1, self.state.agent_coordinate[1]], temp), self, action, self.path_cost + 2, matrix_size))
				elif action == "left" and self.state.agent_coordinate[1] != 0:
					temp = self.state.dirt_list
					child_nodes.append(Node(State([self.state.agent_coordinate[0], self.state.agent_coordinate[1] - 1], temp), self, action, self.path_cost + 2, matrix_size))
