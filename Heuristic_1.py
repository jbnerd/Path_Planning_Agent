		...
		for action in actions:
 		...
 		# 	if [self.get_agent_coordinate()[0] + 1, self.get_agent_coordinate()[1] + 1] in self.state.dirt_list:
			...
			if action == "suck" and self.state.agent_coordinate in self.state.dirt_list:			
				...
				new_node = Node(new_state, self, new_action, new_path_cost, matrix_size, new_heuristic_cost)
				...
				if action == "up" and self.state.agent_coordinate[0] != 0:
					...
					if [self.get_agent_coordinate()[0], self.get_agent_coordinate()[1] - 1] in self.state.dirt_list:
						new_heuristic_cost += -1								
					child_nodes.append(Node(State([self.state.agent_coordinate[0] - 1, self.state.agent_coordinate[1]], temp), self, action, self.path_cost + 2, matrix_size, new_heuristic_cost))
					...
					if [self.get_agent_coordinate()[0] - 1, self.get_agent_coordinate()[1]] in self.state.dirt_list:
						new_heuristic_cost += -1								
					child_nodes.append(Node(State([self.state.agent_coordinate[0], self.state.agent_coordinate[1] - 1], temp), self, action, self.path_cost + 2, matrix_size, new_heuristic_cost))
...
def gbs1(root_state, goal_state, matrix_size):
	...
	while len(frontier) > 0:
		...
		count += len(children)
		children.sort(key = lambda x: x.heuristic_cost)
		for child in children:
			if child.state not in explored:
				if child.state == goal_state:
					return (solution(child, goal_state), count)
				else:
